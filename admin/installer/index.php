...
<div id="container">
    ...
    if (isset($_GET['process']) && !empty($_POST)) {
        $search_baseurl_string = '$firebaseget_Db_URL = "your_key";';
        $replace_baseurl_string = '$firebaseget_Db_URL = "' . $_POST["firebasedburl"] . '";';

        $search_baseurl_domain_string = '$baseurl_domain = "your_key";';
        $replace_baseurl_domain_string = '$baseurl_domain = "' . $_POST["domainurl"] . '";';

        $path_to_file = '../config.php';
        $file_contents = file_get_contents($path_to_file);
        $file_contents = str_replace($search_baseurl_string, "$replace_baseurl_string", $file_contents);

        $file_contents = str_replace($search_baseurl_domain_string, "$replace_baseurl_domain_string", $file_contents);
        ...
        <form method="post" id="nosend" class="newform" action="?process">
            ...
            <input name="domainurl" id="domain" type="text" size=""
                   value=""
...
<script>
  ...
  $('#validating').on('click', function (e) {
    ...
    var newfirabseurlappend = firbaseurl.replace(/\/$/, '')
    var domainurl = $('#domain').val()
    var newdomianurl = domainurl.replace(/\/$/, '') ;
      ...
      success: function (data2) {
        ...
        if (typeof json2.name !== 'undefined') {
          if (json2.name.length > 0) {
            ...
            $('#firebasedburl').val(newfirabseurlappend);
            $('#domain').val(newdomianurl);
            $('#nosend').submit();
          }
        }
      }
